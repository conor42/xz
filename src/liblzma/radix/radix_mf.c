...
#  pragma GCC diagnostic ignored "-Wmaybe-uninitialized" // 'rpt_head_next' may be used uninitialized in this function
#elif defined(_MSC_VER)
#  pragma warning(disable : 4701) // 'rpt_head_next' may be used uninitialized in this function
#endif
...
#define MIN_MATCH_BUFFER_SIZE 256U
// Max buffer size constrained by 24-bit link values.
#define MAX_MATCH_BUFFER_SIZE (1UL << 24)


static void
builder_init_tails(rmf_builder* const tbl)
{
...
static size_t
calc_buf_size(size_t dictionary_size)
{
...
static int
dict_is_struct(size_t const dictionary_size)
{
...
static size_t
dict_allocation_size(size_t dictionary_size, int is_struct)
{
...
static void
init_list_heads(rmf_match_table* const tbl)
{
	for (size_t i = 0; i < RADIX16_TABLE_SIZE; i += 2) {
...
static void
rmf_handle_repeat(rmf_build_match* const match_buffer,
...
{
	if(tbl != NULL)
		lzma_atomic_add(tbl->st_index, RADIX_CANCEL_INDEX - ATOMIC_INITIAL_VALUE);
}
