...
extern void
lzma2_rmf_enc_free(lzma2_rmf_encoder *const enc)
{
	free(enc->hash_buf);
}


static hint_inline unsigned
lzma_rep_1_price(lzma2_rmf_encoder *const restrict enc, size_t const state, size_t const pos_state)
{
	...
	return rc_bit_0_price(rep_G0_prob) + rc_bit_0_price(rep0_long_prob);
}


static unsigned
lzma_rep_price(lzma2_rmf_encoder *const restrict enc, size_t const rep_index, size_t const state, size_t const pos_state)
{
	unsigned price;
	unsigned const rep_G0_prob = enc->states.is_rep_G0[state];
	if (rep_index == 0) {
		unsigned const rep0_long_prob = enc->states.is_rep0_long[state][pos_state];
		price = rc_bit_0_price(rep_G0_prob);
		price += rc_bit_1_price(rep0_long_prob);
	}
	else {
		unsigned const rep_G1_prob = enc->states.is_rep_G1[state];
		price = rc_bit_1_price(rep_G0_prob);
		if (rep_index == 1) {
			price += rc_bit_0_price(rep_G1_prob);
		}
		else {
			unsigned const rep_G2_prob = enc->states.is_rep_G2[state];
			price += rc_bit_1_price(rep_G1_prob);
			price += rc_bit_price(rep_G2_prob, rep_index - 2);
		}
	}
	return price;
}
...
static unsigned
lzma_rep0_price(lzma2_rmf_encoder *const restrict enc, size_t const len, size_t const state, size_t const pos_state)
{
	unsigned const rep_G0_prob = enc->states.is_rep_G0[state];
	unsigned const rep0_long_prob = enc->states.is_rep0_long[state][pos_state];
	return enc->states.rep_len_states.prices[pos_state][len - MATCH_LEN_MIN]
		+ rc_bit_0_price(rep_G0_prob)
		+ rc_bit_1_price(rep0_long_prob);
}


static unsigned
lzma_literal_matched_price(const probability *const restrict prob_table, uint32_t symbol, unsigned match_byte)
{
	unsigned price = 0;
	unsigned offs = 0x100;
	symbol |= 0x100;
	do {
		match_byte <<= 1;
		price += rc_bit_price(prob_table[offs + (match_byte & offs) + (symbol >> 8)], (symbol >> 7) & 1);
		symbol <<= 1;
...
lzma_encode_literal(lzma2_rmf_encoder *const restrict enc, size_t const pos, uint32_t symbol, unsigned const prev_symbol)
{
...
static hint_inline void
lzma_encode_literal_matched(lzma2_rmf_encoder *const restrict enc, const uint8_t* const restrict data_block,
		size_t const pos, uint32_t symbol)
{
	rcf_bit_0(&enc->rc, &enc->states.is_match[enc->states.state][pos & enc->pos_mask]);
	enc->states.state = literal_next_state(enc->states.state);
...
static hint_inline void
lzma_encode_literal_buf(lzma2_rmf_encoder *const restrict enc, const uint8_t* const restrict data_block, size_t const pos)
{
...
static void
lzma_len_set_prices(const probability *restrict probs, uint32_t start_price, unsigned *restrict prices)
{
	for (size_t i = 0; i < 8; i += 2) {
		uint32_t prob = probs[4 + (i >> 1)];
		uint32_t price = start_price + rc_bit_price(probs[1], (i >> 2))
			+ rc_bit_price(probs[2 + (i >> 2)], (i >> 1) & 1);
		prices[i] = price + rc_bit_0_price(prob);
		prices[i + 1] = price + rc_bit_1_price(prob);
	}
}


static force_noinline void
lzma_len_update_prices(lzma2_rmf_encoder *const restrict enc, lzma2_len_states *const restrict ls)
{
	uint32_t b;

	{
		...
		uint32_t a, c;
		b = rc_bit_1_price(prob);
		a = rc_bit_0_price(prob);
		c = b + rc_bit_0_price(ls->low[0]);
	...
	if (i > LEN_LOW_SYMBOLS * 2) {
		...
		do {
			...
			do {
				size_t bit = sym & 1;
				sym >>= 1;
				price += rc_bit_price(probs[sym], bit);
			} while (sym >= 2);

			unsigned const prob = probs[i + (1 << (LEN_HIGH_BITS - 1))];
			prices[i * 2] = price + rc_bit_0_price(prob);
			prices[i * 2 + 1] = price + rc_bit_1_price(prob);
		} while (i);
...
static force_noinline void
lzma_len_encode_mid_high(lzma2_rmf_encoder *const restrict enc, lzma2_len_states* const restrict len_prob_table,
		unsigned const len, size_t const pos_state)
{
	rcf_bit_1(&enc->rc, &len_prob_table->choice);
	if (len < LEN_LOW_SYMBOLS * 2) {
		rcf_bit_0(&enc->rc, &len_prob_table->low[0]);
		rcf_bittree(&enc->rc, len_prob_table->low + LEN_LOW_SYMBOLS + (pos_state << (1 + LEN_LOW_BITS)),
			LEN_LOW_BITS, len - LEN_LOW_SYMBOLS);
	}
	else {
...
static hint_inline void
lzma_len_encode(lzma2_rmf_encoder *const restrict enc, lzma2_len_states* const restrict len_prob_table,
		unsigned len, size_t const pos_state)
{
	len -= MATCH_LEN_MIN;
...
static force_noinline void
lzma_encode_rep_short(lzma2_rmf_encoder *const restrict enc, size_t const pos_state)
{
...
static force_noinline void
lzma_encode_rep_long(lzma2_rmf_encoder *const restrict enc, unsigned const len, unsigned const rep, size_t const pos_state)
{
	rcf_bit_1(&enc->rc, &enc->states.is_match[enc->states.state][pos_state]);
	rcf_bit_1(&enc->rc, &enc->states.is_rep[enc->states.state]);
	if (rep == 0) {
...
static hint_inline void
lzma_encode_normal_match(lzma2_rmf_encoder *const restrict enc, unsigned const len, uint32_t const dist, size_t const pos_state)
{
	rcf_bit_1(&enc->rc, &enc->states.is_match[enc->states.state][pos_state]);
...
static force_inline_template size_t
lzma_encode_chunk_fast(lzma2_rmf_encoder *const restrict enc,
	...
	size_t const uncompressed_end)
{
	...
	while (pos < uncompressed_end && rcf_chunk_size(&enc->rc) < enc->chunk_size) {
		...
		for (rep_match.dist = 0; rep_match.dist < REPS; ++rep_match.dist) {
			...
			rep_match.length = (uint32_t)lzma_memcmplen(data, data_2, 2, max_len);
...
static force_noinline void
lzma_reverse_optimal_chain(lzma2_node* const restrict opt_buf, size_t cur)
{
...
static unsigned
lzma_literal_price(lzma2_rmf_encoder *const restrict enc, size_t const pos, size_t const state,
		unsigned const prev_symbol, uint32_t symbol, unsigned const match_byte)
{
	...
	if (is_lit_state(state)) {
		...
		symbol |= 0x100;
		do {
			price += rc_bit_price(prob_table[symbol >> 8], (symbol >> 7) & 1);
			symbol <<= 1;
		} while (symbol < 0x10000);
		return price;
	}
...
static void
lzma_hash_reset(lzma2_rmf_encoder *const enc, unsigned const dictionary_bits_3)
{
...
static int
lzma_hash_create(lzma2_rmf_encoder *const enc, unsigned const dictionary_bits_3)
{
	if (enc->hash_buf)
		free(enc->hash_buf);
...
lzma_hash_match(lzma2_rmf_encoder *const restrict enc, lzma_data_block const block,
	ptrdiff_t const pos,
	...
	if (first_3 >= 0) {
		...
		if (match_3 >= end_index) {
			do {
				...
				size_t len_test = lzma_memcmplen(data, data_2, 1, length_limit);
				if (len_test > max_len) {
...
static force_inline_template size_t
lzma_optimal_parse(lzma2_rmf_encoder* const restrict enc, lzma_data_block const block,
	rmf_match match,
	size_t const pos,
	size_t const cur,
	size_t len_end,
	...
	uint32_t* const reps)
{
	...
	const uint8_t* const restrict data = block.data + pos;
	...
	{   lzma2_node *const next_opt = &enc->opt_buf[cur + 1];
		...
		uint32_t cur_and_lit_price = cur_price + rc_bit_0_price(is_match_prob);
		...
		match_price = cur_price + rc_bit_1_price(is_match_prob);
		rep_match_price = match_price + rc_bit_1_price(is_rep_prob);
		...
		if (is_hybrid && try_lit && match_byte != cur_byte) {
			...
			size_t len_test_2 = lzma_memcmplen(data + 1, data_2, 0, limit);
			if (len_test_2 >= 2) {
				...
				uint32_t const next_rep_match_price = cur_and_lit_price +
					rc_bit_1_price(enc->states.is_match[state_2][pos_state_next]) +
					rc_bit_1_price(enc->states.is_rep[state_2]);
	...
	if (match.length > 0) {
		...
		for (size_t rep_index = 0; rep_index < REPS; ++rep_index) {
			...
			len_test = lzma_memcmplen(data, data_2, 2, max_length);
			...
			if (is_hybrid && len_test + 3 <= bytes_avail && !not_equal_16(data + len_test + 1, data_2 + len_test + 1)) {
				...
				size_t const len_test_2 = lzma_memcmplen(data + len_test + 1, data_2 + len_test + 1,
					2, my_min(fast_length, bytes_avail - len_test - 1));
				...
				uint32_t rep_lit_rep_total_price =
					...
					+ rc_bit_0_price(enc->states.is_match[state_2][pos_state_next])
	...
	if (match.length >= start_len && max_length >= start_len) {
		// Try normal match 
		uint32_t const normal_match_price = match_price + rc_bit_0_price(is_rep_prob);
		if (!is_hybrid) {
			...
			for (; match_index >= start_match; --match_index) {
				...
				for (; len_test >= base_len; --len_test) {
					...
					if (!sub_len && rep_0_pos + 2 <= bytes_avail && !not_equal_16(data + rep_0_pos, data_2 + rep_0_pos)) {
						...
						size_t const limit = my_min(fast_length, bytes_avail - rep_0_pos);
						size_t const len_test_2 = lzma_memcmplen(data + rep_0_pos, data_2 + rep_0_pos, 2, limit);
...
static force_noinline void
lzma_init_matches_pos0(lzma2_rmf_encoder *const restrict enc,
	rmf_match const match,
...
static force_noinline size_t
lzma_init_matches_pos0_best(lzma2_rmf_encoder *const restrict enc, lzma_data_block const block,
	rmf_match const match,
	size_t const pos,
	size_t start_len,
	unsigned const normal_match_price)
...
// available.
static force_inline_template size_t
lzma_init_optimizer_pos0(lzma2_rmf_encoder *const restrict enc, lzma_data_block const block,
	rmf_match const match,
	size_t const pos,
	int const is_hybrid,
	uint32_t* const reps)
{
	...
	for (size_t i = 0; i < REPS; ++i) {
		...
		rep_lens[i] = lzma_memcmplen(data, data_2, 2, max_length);
	...
	enc->opt_buf[1].price = rc_bit_0_price(is_match_prob) +
		lzma_literal_price(enc, pos, state, data[-1], cur_byte, match_byte);
	mark_literal(enc->opt_buf[1]);

	unsigned const match_price = rc_bit_1_price(is_match_prob);
	unsigned const rep_match_price = match_price + rc_bit_1_price(is_rep_prob);
	if (match_byte == cur_byte) {
		// Try 1-byte rep0 
	...
	unsigned const normal_match_price = match_price + rc_bit_0_price(is_rep_prob);
...
static force_inline_template size_t
lzma_encode_opt_sequence(lzma2_rmf_encoder *const restrict enc, lzma_data_block const block,
	rmf_match_table* const restrict tbl,
	int const struct_tbl,
	int const is_hybrid,
	size_t start_index,
...
static void force_noinline
lzma_fill_align_prices(lzma2_rmf_encoder *const restrict enc)
{
	unsigned i;
	...
	for (i = 0; i < ALIGN_SIZE / 2; i++) {
		...
		unsigned m = 1;
		unsigned bit;
		bit = sym & 1; sym >>= 1; price += rc_bit_price(probs[m], bit); m = (m << 1) + bit;
		bit = sym & 1; sym >>= 1; price += rc_bit_price(probs[m], bit); m = (m << 1) + bit;
		bit = sym & 1; sym >>= 1; price += rc_bit_price(probs[m], bit); m = (m << 1) + bit;
		uint32_t const prob = probs[m];
		enc->align_prices[i] = price + rc_bit_0_price(prob);
		enc->align_prices[i + 8] = price + rc_bit_1_price(prob);
	}
}


static void force_noinline
lzma_fill_dist_prices(lzma2_rmf_encoder *const restrict enc)
{
	...
	for (size_t i = DIST_MODEL_START / 2; i < FULL_DISTANCES / 2; i++) {
		...
		for (; footer_bits != 0; --footer_bits) {
			unsigned bit = sym & 1;
			sym >>= 1;
			price += rc_bit_price(probs[m], bit);
			m = (m << 1) + bit;
		};

		unsigned const prob = probs[m];
		temp_prices[base] = price + rc_bit_0_price(prob);
		temp_prices[base + offset] = price + rc_bit_1_price(prob);
	}
	...
	for (unsigned lps = 0; lps < DIST_STATES; lps++) {
		...
		for (slot = 0; slot < dist_table_size2; slot++) {
			...
			bit = sym & 1; sym >>= 1; price = rc_bit_price(probs[sym], bit);
			bit = sym & 1; sym >>= 1; price += rc_bit_price(probs[sym], bit);
			bit = sym & 1; sym >>= 1; price += rc_bit_price(probs[sym], bit);
			bit = sym & 1; sym >>= 1; price += rc_bit_price(probs[sym], bit);
			bit = sym & 1; sym >>= 1; price += rc_bit_price(probs[sym], bit);
			unsigned const prob = probs[slot + (1 << (DIST_SLOT_BITS - 1))];
			dist_slot_prices[slot * 2] = price + rc_bit_0_price(prob);
			dist_slot_prices[slot * 2 + 1] = price + rc_bit_1_price(prob);
		}
...
static force_inline_template size_t
lzma_encode_chunk_best(lzma2_rmf_encoder *const restrict enc,
	lzma_data_block const block,
	rmf_match_table* const restrict tbl,
	int const struct_tbl,
	size_t pos,
...
static void
lzma_len_probs_reset(lzma2_len_states* const ls, unsigned const fast_length)
{
...
static void
lzma_probs_reset(lzma2_rmf_encoder *const restrict enc)
{
...
extern size_t
lzma2_enc_rmf_mem_usage(unsigned const chain_log, lzma_mode const strategy, unsigned const thread_count)
{
...
static void
lzma2_reset(lzma2_rmf_encoder *const restrict enc, size_t const max_distance)
{
	rcf_reset(&enc->rc);
...
static uint8_t
lzma2_is_chunk_incompressible(const rmf_match_table* const restrict tbl,
		...
		if (tbl->is_struct) {
			...
			for (size_t pos = start; pos < end; ) {
				...
				if (link == RADIX_NULL_LINK) {
					...
					size_t const dist = pos - get_match_link(tbl->table, pos);
					if (length > 4) {
...
static size_t
lzma2_encode_chunk(lzma2_rmf_encoder *const restrict enc,
	rmf_match_table* const restrict tbl,
	lzma_data_block const block,
...
extern size_t
lzma2_rmf_encode(lzma2_rmf_encoder *const restrict enc,
		rmf_match_table *const restrict tbl,
		lzma_data_block const block,
		...
		bool *const canceled)
{
	...
	for (size_t pos = start; pos < block.end;) {
		...
		if (incompressible || uncompressed_size + 3 <= compressed_size
				+ (compressed_size >> RANDOM_FILTER_MARGIN_BITS) + header_size) {
		...
		lzma_atomic_add(*progress_in, (long)(next_index - pos));
		lzma_atomic_add(*progress_out, (long)(compressed_size + header_size));
